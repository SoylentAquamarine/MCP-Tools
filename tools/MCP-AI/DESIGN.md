...
1. Add `provider/openai/api-key` to the Hub-managed `secrets.json` (see `docs/HUB-SECRETS.md`).
2. Add OpenAI provider config with `secretRef`.
3. Run `ai_health_check` from the Hub, which resolves the ref and passes the value at runtime.
