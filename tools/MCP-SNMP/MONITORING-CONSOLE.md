...
`MCP-SNMP` should be able to turn SNMP discovery into a monitoring console definition.
...
```text
1. Claude asks MCP-SNMP to discover a device.
2. MCP-SNMP walks useful SNMP tables.
3. MCP-SNMP identifies counters worth monitoring.
4. MCP-SNMP builds a monitoring profile.
5. MCP-SNMP writes console JSON/Markdown to a job folder.
6. Claude or MCP-Hub uses that output.
```
...
`MCP-SNMP` should create the model and recommendations.

A separate MCP-Hub or monitoring system should render the long-running dashboard and perform continuous polling.
